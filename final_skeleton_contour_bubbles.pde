...
ArrayList<Particle2> particles3 = new ArrayList<Particle2>();
ArrayList<Particle2> particles4 = new ArrayList<Particle2>();
void setup() {
  size(2000, 1000, P3D);
  smooth();
...
void draw() {
  ...
  if (skeletonArray.size() > 0) {
   ...
   if (skeleton.isTracked()) {
     ...
     strokeWeight(0.1);
     //drawLine
     KJoint joint1 = joints[KinectPV2.JointType_HandRight];
     PVector jointhand = joint1.getPosition();
     pushMatrix();
     noStroke();
     fill(255, 100); 
     popMatrix();
   ...
   if (contour.numPoints() > 50) { 
     ...
     p+=0.005;
      o+=0.005;
       e+=0.005;
       r+=0.005;
     if(p>=2.5)p=1; if(o>=2.5)o=1; if(e>=2.5)e=1;if(r>=2.5)r=1;
   }
  }
  }
   pushMatrix();
